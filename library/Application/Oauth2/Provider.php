...
class Application_Oauth2_Provider {
	 ...
	 * @throws Application_Oauth2_Provider_Exception
	 * 
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-20#section-4
	...
	public function grantAccessToken(array $inputData = NULL, array $authHeaders = NULL) {
		...
		if (!$input["grant_type"]) {
			throw new Application_Oauth2_Provider_Exception(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'Invalid grant_type parameter or parameter missing');
		}
		...
		switch ($input["grant_type"]) {
			case self::GRANT_TYPE_AUTH_CODE:
				...
				if ($stored === NULL || $client[0] != $stored["client_id"]) {
					throw new Application_Oauth2_Provider_Exception(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT, "Refresh token doesn't exist or is invalid for the client");
				}
				...
				if ($input["redirect_uri"] && !$this->validateRedirectUri($input["redirect_uri"], $stored["redirect_uri"])) {
					throw new Application_Oauth2_Provider_Exception(self::HTTP_BAD_REQUEST, self::ERROR_REDIRECT_URI_MISMATCH, "The redirect URI is missing or do not match");
				}
				
				if ($stored["expires"] < time()) {
					throw new Application_Oauth2_Provider_Exception(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT, "The authorization code has expired");
				}
				break;
			...
			case self::GRANT_TYPE_REFRESH_TOKEN:
				...
				if ($stored["expires"] < time()) {
					throw new Application_Oauth2_Provider_Exception(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT, 'Refresh token has expired');
				}
		...
		if ($input["scope"] && (!is_array($stored) || !isset($stored["scope"]) || !$this->checkScope($input["scope"], $stored["scope"]))) {
			throw new Application_Oauth2_Provider_Exception(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_SCOPE, 'An unsupported scope was requested.');
		}
	...
	protected function getClientCredentials(array $inputData, array $authHeaders) {
		...
		if (!empty($authHeaders['PHP_AUTH_USER'])) {
			...
			throw new Application_Oauth2_Provider_Exception(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_CLIENT, 'Client id was not found in the headers or body');
		} else {
	...
	public function getAuthorizeParams(array $inputData = NULL) {
		...
		if (!$input["client_id"]) {
			throw new Application_Oauth2_Provider_Exception(self::HTTP_BAD_REQUEST, self::ERROR_INVALID_CLIENT, "No client id supplied"); // We don't have a good URI to use
		}
		...
		if ($stored["redirect_uri"] && $input["redirect_uri"] && !$this->validateRedirectUri($input["redirect_uri"], $stored["redirect_uri"])) {
			throw new Application_Oauth2_Provider_Exception(self::HTTP_BAD_REQUEST, self::ERROR_REDIRECT_URI_MISMATCH, 'The redirect URI provided is missing or does not match');
		}
